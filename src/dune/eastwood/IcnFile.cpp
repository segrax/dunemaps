...
size_t	IcnFile::getTileCount( uint32_t index ) {
	...
	return (*m_tileSet)[index].size();
}

std::vector<size_t> IcnFile::getTileIDs( uint32_t index ) {
	std::vector<size_t>				tileIDs;
	std::vector<uint16_t>::iterator	tileIT;

	for( tileIT = (*m_tileSet)[index].begin(); tileIT != (*m_tileSet)[index].end(); ++tileIT ) {

		tileIDs.push_back( *tileIT );
	}
